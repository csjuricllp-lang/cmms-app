...
export const MobileSettings = () => {
    ...
    return (
        <div className="flex flex-col h-full bg-background font-outfit relative overflow-hidden">

            {/* Top Header Bar */}
            <div className="bg-card border-b border-slate-100 sticky top-0 z-30 shadow-sm shrink-0 px-4 py-4 flex items-center justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                    <div className="w-8 h-8 rounded-xl bg-primary/10 border border-primary/10 flex items-center justify-center text-primary shrink-0">
                        <Settings2 className="w-4 h-4" />
                    </div>
                    <div className="min-w-0">
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-none">Settings</p>
                        <h1 className="text-[16px] font-black text-foreground tracking-tight truncate">{activeItem?.label ?? 'Organization'}</h1>
                    </div>
                </div>
                ...
                <button
                    ...
                    className="shrink-0 flex items-center gap-2 px-3 py-2 bg-transparent border border-border rounded-xl text-[12px] font-bold text-slate-600 active:scale-95 transition-transform"
                >
                    <ActiveIcon className="w-3.5 h-3.5 text-primary/80" />
                    Change
            ...
            <div className="fixed bottom-0 left-0 right-0 z-30 bg-card border-t border-slate-100 px-4 py-3 flex items-center gap-3 shadow-[0_-4px_24px_rgba(0,0,0,0.04)]">
                <button className="flex-1 h-11 bg-transparent border border-border text-slate-600 text-[13px] font-black rounded-xl hover:bg-muted transition-all active:scale-95">
                    Back to Registry
                </button>
                <button className="flex-1 h-11 bg-primary text-white text-[13px] font-black rounded-xl shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all active:scale-95">
                    Save Changes
                </button>
            </div>
                ...
                {isNavOpen && (
                    <>
                        ...
                        <motion.div
                            ...
                            className="fixed bottom-0 left-0 right-0 z-50 bg-card rounded-t-3xl shadow-2xl max-h-[82vh] flex flex-col"
                        >
                            {/* Sheet Handle */}
                            ...
                            <div className="px-5 py-3 flex items-center justify-between border-b border-slate-100 shrink-0">
                                <h2 className="text-[16px] font-black text-foreground">Settings Modules</h2>
                                <button
                                    onClick={() => setIsNavOpen(false)}
