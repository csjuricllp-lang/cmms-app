...
const SHIFT_COLORS = [
    { bg: 'bg-primary/10', border: 'border-primary/10', text: 'text-primary/90', dot: 'bg-primary/80', badge: 'bg-primary/15 text-primary/90' },
        ...
        {isOpen && (
            <div className="fixed inset-0 z-[100] flex flex-col justify-end">
                ...
                <motion.div
                    ...
                    className="relative bg-card rounded-t-3xl shadow-2xl z-10 overflow-hidden"
                >
                    {/* Sheet Header */}
                    <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-transparent">
                        ...
                        <button
                            onClick={onClose}
                            className="w-8 h-8 rounded-full bg-slate-200 flex items-center justify-center text-muted-foreground active:scale-90 transition-transform"
                        >
                            <X className="w-4 h-4" />
                        </button>
                    </div>
                    ...
                    <div className="px-5 py-5 space-y-5">
                        ...
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Shift Name</label>
                            <input
                                type="text"
                                value={name}
                                onChange={e => onNameChange(e.target.value)}
                                placeholder="e.g. Morning Shift"
                                className="w-full px-4 py-3 bg-transparent border border-border rounded-xl text-[14px] font-bold text-foreground outline-none focus:bg-card focus:border-primary/80/50 focus:ring-4 focus:ring-primary/5 transition-all"
                            />
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div className="space-y-1.5">
                                ...
                                <input
                                    ...
                                    className="w-full px-4 py-3 bg-transparent border border-border rounded-xl text-[14px] font-bold text-foreground outline-none focus:bg-card focus:border-primary/80/50 transition-all"
                                />
                            </div>
                            <div className="space-y-1.5">
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Work Days</label>
                    ...
                    {/* Footer */}
                    <div className="px-5 py-4 bg-transparent border-t border-slate-100 flex items-center gap-3">
                        <button
                            onClick={onClose}
                            className="flex-1 h-11 bg-card border border-border text-slate-600 text-[13px] font-black rounded-xl transition-all active:scale-95"
                        >
                            Cancel
                        </button>
                        <button
                            onClick={onSave}
                            disabled={isSaving}
                            className="flex-1 h-11 bg-primary hover:bg-primary/90 text-white text-[13px] font-black rounded-xl shadow-lg shadow-primary/20 transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
                        >
            ...
            <div className="bg-card px-4 py-4 border-b border-slate-100 sticky top-0 z-30 shadow-sm shrink-0 space-y-3">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                        ...
                        <h1 className="text-[17px] font-black text-foreground tracking-tight">Shifts Management</h1>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                            ...
                            {(['List', 'Calendar'] as const).map((view) => (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        currentView === view ? "bg-card text-primary shadow-sm" : "text-slate-400"
                                    )}
                                >
                        ...
                        <button
                            ...
                            className="flex items-center gap-1.5 px-3 py-2 bg-primary hover:bg-primary/90 text-white rounded-xl text-[12px] font-black shadow-lg shadow-primary/20 active:scale-95 transition-transform"
                        >
                ...
                {isLoading ? (
                    ...
                    <div className="space-y-3">
                        {filteredShifts.length === 0 ? (
                            <div className="bg-card rounded-3xl border border-slate-100 p-12 text-center shadow-sm space-y-4">
                                <div className="w-14 h-14 bg-transparent rounded-2xl border border-slate-100 flex items-center justify-center mx-auto">
                                    <Clock className="w-7 h-7 text-slate-200" />
                                </div>
                                <div className="space-y-1">
                                    <p className="text-[15px] font-black text-foreground/90">No Shifts Found</p>
                                    ...
                                    >
                                        ...
                                        <div className="flex items-start justify-between gap-2">
                                            <div className="flex items-center gap-3 min-w-0">
                                                ...
                                                <h3 className={cn("text-[15px] font-black leading-tight truncate", isDark ? 'text-white' : 'text-foreground')}>
                                                    {shift.name}
                                                </h3>
                                            </div>
                                            <div className="flex items-center gap-1 shrink-0">
                                                <button
                                                    ...
                                                    className={cn("p-1.5 rounded-lg transition-colors", isDark ? 'hover:bg-white/10 text-slate-300' : 'hover:bg-card text-slate-400 hover:text-primary')}
                                                >
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className={cn("text-[13px] font-black", isDark ? 'text-slate-200' : 'text-foreground/90')}>
                                        ...
                                        <div className="flex items-center gap-2 pt-1 border-t border-white/20">
                                            ...
                                            <span className={cn("text-[11px] font-bold", isDark ? 'text-slate-400' : 'text-muted-foreground')}>
                    ...
                    <div className="space-y-3">
                        <div className="bg-card rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                            {/* Day headers */}
                            <div className="grid grid-cols-7 border-b border-slate-100 bg-transparent">
                                {DAYS.map((day) => (
                                    <div key={day} className="py-2.5 text-center text-[11px] font-black text-muted-foreground uppercase tracking-wider">
                                        {day.charAt(0)}
                                    </div>
                                ))}
                            </div>
                        ...
                        {/* Legend */}
                        <div className="bg-card rounded-2xl border border-slate-100 shadow-sm p-4 space-y-2">
                            <p className="text-[11px] font-black text-muted-foreground uppercase tracking-wider">Shift Legend</p>
                            {filteredShifts.map((shift, idx) => {
                                const color = SHIFT_COLORS[idx % SHIFT_COLORS.length];
                                return (
                                    <div key={shift.id} className="flex items-center gap-3">
                                        <div className={cn("w-3 h-3 rounded-full", color.dot)} />
                                        <span className="text-[13px] font-bold text-foreground/90">{shift.name}</span>
