...
export const MobileLocations = ({
  ...
  return (
    <div className="flex flex-col h-full min-h-screen bg-transparent font-outfit select-none relative pb-20">
      {/* Header Bar */}
      <div className="bg-card px-4 py-4 border-b border-slate-100 sticky top-0 z-30 shadow-sm flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 relative">
            <div className="w-8 h-8 rounded-xl bg-primary/10 border border-primary/10 flex items-center justify-center text-indigo-650">
              <MapPin className="w-4.5 h-4.5" />
            </div>
            <div className="flex items-center gap-1 cursor-pointer" onClick={() => setIsSavedViewsOpen(!isSavedViewsOpen)}>
              <h1 className="text-[17px] font-black text-foreground tracking-tight">Locations</h1>
            ...
            {isSavedViewsOpen && (
              <>
                ...
                <div className="absolute top-full left-0 mt-2 w-48 bg-card border border-slate-100 rounded-2xl shadow-xl z-50 p-1.5 animate-in fade-in slide-in-from-top-2">
                  ...
                  <button
                    ...
                    className="w-full text-left px-3 py-2 rounded-xl text-xs font-bold text-primary bg-primary/10/15 hover:bg-primary/10/20 transition-all flex items-center justify-between"
                  >
                    Default View
                  ...
                  {savedViews.map((view) => (
                    <button
                      ...
                      className="w-full text-left px-3 py-2 rounded-xl text-xs font-bold text-slate-600 hover:bg-transparent transition-all mt-0.5"
                    >
                      {view.name}
                    </button>
                  ))}
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-2 hover:bg-transparent border border-slate-200/60 rounded-xl text-muted-foreground hover:text-foreground/90 transition-colors active:scale-95 shadow-sm"
              title="Export CSV"
            >
            ...
            <button
              ...
              className="p-2 hover:bg-transparent border border-slate-200/60 rounded-xl text-muted-foreground hover:text-foreground/90 transition-colors active:scale-95 shadow-sm flex items-center gap-1.5 text-xs font-bold"
        ...
        <div className="flex gap-2 items-center">
          ...
          <button
            ...
            className={cn(
              ...
              hasActiveFilters
                ? "border-primary text-indigo-650 bg-primary/10/10"
      ...
      {activeTab === 'List' ? (
        <>
          {/* Filter Tabs */}
          <div className="px-4 py-3 flex gap-2 overflow-x-auto scrollbar-none bg-transparent border-b border-slate-100/50 shrink-0">
            {[
          ...
          <div className="flex-1 px-4 py-3 overflow-y-auto">
            {isLoading ? (
              ...
              <div className="bg-card rounded-3xl border border-border p-12 text-center my-6 space-y-4 shadow-sm">
                ...
                <div className="space-y-1">
                  <p className="text-[14px] font-black text-foreground/90">No Locations Found</p>
                ...
                {filteredLocations.map((loc) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="bg-card border border-slate-200/80 rounded-2xl p-4 flex gap-4 items-start shadow-sm active:bg-transparent/50 active:scale-[0.99] transition-all cursor-pointer font-sans relative"
                    >
                      <div className="w-12 h-12 rounded-xl bg-primary/10 border border-primary/10 flex items-center justify-center text-indigo-650 shrink-0 shadow-inner">
                      ...
                      <div className="flex-1 min-w-0 space-y-2">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <button
                            ...
                            className="p-1 hover:bg-slate-105 border border-slate-200/40 rounded-xl text-slate-400 hover:text-foreground/90 transition-colors shrink-0 -mt-1 -mr-1 shadow-sm flex items-center justify-center"
                          >
                        ...
                        <div className="flex flex-wrap items-center gap-2 pt-1">
                          {totalSub > 0 && visibleColumnIds.includes('No. of Children') && (
                            <span className="flex items-center gap-1 bg-transparent text-[10px] font-bold text-muted-foreground border border-slate-150 px-2 py-0.5 rounded-lg">
                              <Network className="w-3.5 h-3.5 text-slate-400" /> {totalSub} sub-areas
                            </span>
                          )}
                          {totalWorkers > 0 && visibleColumnIds.includes('Workers') && (
                            <span className="flex items-center gap-1 bg-primary/10/50 text-[10px] font-bold text-primary border border-primary/10 px-2 py-0.5 rounded-lg">
            ...
            >
                    ...
                    <div className="mt-1 border border-border rounded-2xl bg-card overflow-hidden max-h-48 overflow-y-auto custom-scrollbar shadow-lg animate-in fade-in slide-in-from-top-1 z-30">
                      <button
                        ...
                        className={cn(
                          ...
                          activeFilters.selectedAssignees?.length === 0 ? "bg-primary/10/20 text-primary font-black" : "text-muted-foreground hover:bg-muted/10"
                        )}
                      >
                        All Workers
                      </button>
                      <button
                        ...
                        className={cn(
                          "w-full text-left px-4 py-3 text-xs font-bold transition-colors border-b border-border/30 last:border-0",
                          activeFilters.selectedAssignees?.includes('unassigned') ? "bg-primary/10/20 text-primary font-black" : "text-muted-foreground hover:bg-muted/10 hover:text-foreground"
                        )}
                      >
                        Unassigned
                      </button>
                      {users.map(u => {
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              "w-full text-left px-4 py-3 text-xs font-bold transition-colors border-b border-border/30 last:border-0",
                              isSelected ? "bg-primary/10/20 text-primary font-black" : "text-muted-foreground hover:bg-muted/10 hover:text-foreground"
                            )}
                          >
                            {u.name}
                          </button>
                  ...
                  {isTeamDropdownOpen && (
                    <div className="mt-1 border border-border rounded-2xl bg-card overflow-hidden max-h-48 overflow-y-auto custom-scrollbar shadow-lg animate-in fade-in slide-in-from-top-1 z-30">
                      <button
                        ...
                        className={cn(
                          "w-full text-left px-4 py-3 text-xs font-bold transition-colors border-b border-border/30 last:border-0",
                          activeFilters.selectedTeams?.length === 0 ? "bg-primary/10/20 text-primary font-black" : "text-muted-foreground hover:bg-muted/10"
                        )}
                      >
                        All Teams
                      </button>
                  ...
                  {isCustomerDropdownOpen && (
                    <div className="mt-1 border border-border rounded-2xl bg-card overflow-hidden max-h-48 overflow-y-auto custom-scrollbar shadow-lg animate-in fade-in slide-in-from-top-1 z-30">
                      <button
                        ...
                        className={cn(
                          "w-full text-left px-4 py-3 text-xs font-bold transition-colors border-b border-border/30 last:border-0",
                          !activeFilters.customerId ? "bg-primary/10/20 text-primary font-black" : "text-muted-foreground hover:bg-muted/10"
                        )}
                      >
                        All Customers
                      </button>
                      {customers.map(c => (
                        <button
                          ...
                          className={cn(
                            "w-full text-left px-4 py-3 text-xs font-bold transition-colors border-b border-border/30 last:border-0",
                            activeFilters.customerId === c.id ? "bg-primary/10/20 text-primary font-black" : "text-muted-foreground hover:bg-muted/10 hover:text-foreground"
                          )}
                        >
                          {c.name}
                        </button>
                  ...
                  {isVendorDropdownOpen && (
                    <div className="mt-1 border border-border rounded-2xl bg-card overflow-hidden max-h-48 overflow-y-auto custom-scrollbar shadow-lg animate-in fade-in slide-in-from-top-1 z-30">
                      <button
                        ...
                        className={cn(
                          "w-full text-left px-4 py-3 text-xs font-bold transition-colors border-b border-border/30 last:border-0",
                          activeFilters.vendorIds?.length === 0 ? "bg-primary/10/20 text-primary font-black" : "text-muted-foreground hover:bg-muted/10"
                        )}
                      >
                        All Vendors
                      </button>
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            isVisible
                              ? "bg-primary/10 border-primary/20 text-primary/90 font-bold"
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className={cn(
                          ...
                          sortOrder === 'asc'
                            ? "bg-primary/10 border-indigo-205 text-primary/90"
                            : "bg-card border-border text-muted-foreground"
                        )}
                      >
                        Ascending
              ...
              <div className="p-4 border-t border-border bg-card flex flex-col gap-3 shrink-0">
                ...
                <button
                  ...
                  className="w-full py-2.5 bg-transparent border border-border text-slate-650 hover:bg-muted text-xs font-black uppercase tracking-wider rounded-xl transition-all active:scale-95 text-center flex items-center justify-center gap-1.5"
                >
                  Save Current View
                </button>
              </div>
        ...
        {isActionsDrawerOpen && selectedLocationForActions && (
          <div className="fixed inset-0 z-50 flex flex-col justify-end bg-black/60 backdrop-blur-sm">
            ...
            <motion.div
              ...
              exit={{ y: '100%' }}
              transition={{ type: 'spring', damping: 25, stiffness: 220 }}
              className="w-full bg-card border-t border-slate-100 rounded-t-[28px] flex flex-col overflow-hidden shadow-2xl z-50 p-6 space-y-4"
            >
              {/* Drawer Header */}
              <div className="flex items-center justify-between pb-2 border-b border-slate-50">
                <div className="min-w-0">
                  <h2 className="text-[15px] font-black text-foreground truncate">
              ...
              <div className="space-y-1">
                <button
                  ...
                  className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-transparent rounded-xl transition-colors text-left"
                >
                  <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center text-indigo-650">
                    <SlidersHorizontal className="w-4 h-4" />
                  </div>
