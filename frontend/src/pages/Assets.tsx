...
export const AssetsPage = () => {
    ...
    const getStatusStyle = (status: string) => {
        switch (status?.toUpperCase()) {
            ...
            case 'STANDBY': return 'bg-blue-50 text-primary/90 border-blue-100';
            default: return 'bg-muted/50 text-foreground/90 border-gray-100';
        }
    };
    ...
    const renderLocationItem = (loc: any, depth = 0) => {
        ...
        return (
            <div key={loc.id}>
                <div 
                    className={`flex items-center gap-2 py-2 px-3 rounded-md hover:bg-muted/50 cursor-pointer transition-colors ${isSelected ? 'bg-blue-50' : ''}`}
                ...
                >
                    ...
                    />
                    <MapPin className={`w-4 h-4 ${isSelected ? 'text-primary' : 'text-gray-400'}`} />
                    <span className={`text-sm ${isSelected ? 'font-semibold text-primary/90' : 'text-foreground/90'}`}>{loc.name}</span>
                </div>
    ...
    return (
        <>
            ...
            {/* Header Area */}
            <div className="flex items-center justify-between px-8 py-4 border-b border-border bg-card z-20 overflow-x-auto no-scrollbar py-2">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button 
                        ...
                        className="flex items-center gap-2 px-3 py-2 text-sm font-semibold text-foreground/90 hover:bg-muted rounded-md transition-colors"
                    >
                    ...
                    {canManageAssets && (
                        <div className="flex items-center gap-2">
                            <button 
                                ...
                                className="flex items-center gap-2 px-4 py-2 bg-primary hover:bg-primary/90 text-white rounded-md text-sm font-semibold transition-all shadow-sm active:scale-95"
                            >
                            ...
                            <button 
                                ...
                                className="bg-card border border-border p-2 rounded-md hover:bg-muted/50 transition-colors shadow-sm"
                            >
            ...
            {/* Filter Bar */}
            <div className="flex items-center gap-4 px-8 py-3 bg-card border-b border-border z-10 overflow-x-auto no-scrollbar py-2">
                <button 
                    onClick={() => setIsFiltersModalOpen(true)}
                    className="flex items-center gap-2 px-4 py-1.5 bg-primary/10 border border-primary/20 text-primary rounded-md text-[14px] font-semibold hover:bg-primary/20 transition-colors shrink-0"
                >
                ...
                <div className="relative">
                    <button 
                        ...
                        className={`flex items-center gap-2 px-4 py-1.5 bg-card border border-gray-300 rounded-md text-[14px] font-semibold transition-colors hover:bg-muted/50 ${selectedLocationIds.length > 0 ? 'border-primary text-primary/90 bg-blue-50' : 'text-foreground/90'}`}
                    >
                        Location
                    ...
                    {selectedAssetIds.length > 0 && (
                        <div className="flex items-center gap-2 ml-4 px-2 py-1 bg-blue-50/50 rounded-lg border border-blue-100">
                            ...
                            <button
                                ...
                                className="flex items-center gap-1.5 px-3 py-1 bg-card hover:bg-red-50 text-red-600 rounded-md text-[13px] font-bold transition-all border border-transparent hover:border-red-200 shadow-sm"
                            >
                ...
                <div className="flex items-center gap-2 flex-1">
                    {activeFilters.map((filter, idx: number) => (
                        <div key={idx} className="flex items-center gap-2 px-3 py-1 bg-blue-50 border border-blue-100 rounded-full text-[13px] text-primary/90 font-medium">
                    ...
                    {selectedLocationIds.length > 0 && (
                        <div className="flex items-center gap-2 px-3 py-1 bg-blue-50 border border-blue-100 rounded-full text-[13px] text-primary/90 font-medium">
                    ...
                    <button 
                        ...
                        className="text-sm font-semibold text-primary hover:text-blue-800 transition-colors ml-2"
                    >
                        Reset Filters
                    </button>
                </div>
                ...
                <div className="flex items-center gap-4">
                    <button className="text-sm font-semibold text-muted-foreground hover:text-foreground/90 transition-colors">Save View</button>
                </div>
            </div>

            {/* Table Area */}
            <div className="flex-1 overflow-auto bg-background p-6 pt-2">
                <div className="bg-card border border-border rounded-lg shadow-sm overflow-hidden min-w-[1400px]">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-border bg-card">
                                ...
                                {ALL_COLUMNS.filter((col: any) => visibleColumnIds.includes(col.id)).map((col: any) => (
                                    <th key={col.id} className="px-5 py-4 text-[13px] font-bold text-foreground uppercase tracking-tight whitespace-nowrap">
                                        {col.label}
                                    </th>
                                ))}
                            </tr>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {isLoading ? (
                                Array(8).fill(0).map((_, i: number) => (
                                    <tr key={i} className="animate-pulse">
                                        <td className="p-5"><div className="h-4 w-4 bg-muted rounded" /></td>
                                        <td className="p-5" colSpan={visibleColumnIds.length}><div className="h-4 bg-muted/50 rounded w-full" /></td>
                                    </tr>
                                ))
                            ) : assets.length === 0 ? (
                                <TableEmptyState
                            ...
                            return (
                                <tr 
                                    key={asset.id} 
                                    className={`transition-colors group cursor-pointer ${isSelected ? 'bg-blue-50/50 hover:bg-blue-50/80' : 'hover:bg-background'}`}
                                    onClick={() => navigate(`/assets/${asset.id}`)}
                                >
                                    <td className="px-5 py-4 text-center sticky left-0 z-10 border-r border-gray-100 bg-card group-hover:bg-gray-50/50 transition-colors">
                                        <input 
                                            type="checkbox" 
                                            checked={isSelected}
                                            onChange={(e) => {
                                        ...
                                        <td key={col.id} className="px-5 py-4">
                                            {col.id === 'Name' && (
                                                <div className="flex items-center gap-3">
                                                    <div className="w-9 h-9 rounded-lg bg-muted/50 border border-gray-100 flex items-center justify-center text-gray-400 group-hover:text-primary transition-colors">
                                                        <Box className="w-5 h-5" />
                                                    </div>
                                                    <div>
                                                        <span className="text-sm font-bold text-foreground group-hover:text-primary transition-colors">{asset.name}</span>
                                                    </div>
                                                </div>
                                            )}
                                            {col.id === 'Image' && (
                                                asset.imageUrl ? (
                                                    ...
                                                    <div className="w-10 h-10 rounded-lg bg-muted/50 border border-gray-100 flex items-center justify-center">
                ...
                {hasNextPage && (
                    <div className="py-8 flex justify-center">
                        <button
                            ...
                            className="px-6 py-2.5 bg-card border border-border rounded-xl text-[13px] font-black text-gray-600 hover:border-primary/80 hover:text-primary transition-all disabled:opacity-50 shadow-sm flex items-center gap-2"
                        >
                            {isFetchingNextPage ? (
                                <>
            ...
            <AnimatePresence>
                {isFiltersModalOpen && (
                    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} className="relative w-full max-w-[600px] bg-card rounded-xl shadow-[0_40px_120px_-20px_rgba(0,0,0,0.3)] flex flex-col overflow-visible" onClick={(e) => e.stopPropagation()}>
                            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
                                <h2 className="text-[22px] font-black text-[#1F2937] tracking-tight">Filters</h2>
                                <button onClick={() => setIsFiltersModalOpen(false)} className="text-gray-400 hover:text-foreground transition-colors"><X className="w-5 h-5" /></button>
                            </div>
                            <div className="flex-1 p-6 min-h-[160px] flex flex-col">
                                {activeFilters.length === 0 ? (
                                    <div className="flex flex-col items-center justify-center flex-1 py-12 text-center text-muted-foreground">
                                        ...
                                        <h3 className="text-base font-bold text-foreground mb-1">No active filters.</h3>
                                    ...
                                    <div className="flex flex-col gap-4">
                                        {activeFilters.map((filter: any, index: number) => (
                                            <div key={index} className="space-y-2 group animate-in slide-in-from-top-2">
                                                ...
                                                <div className="relative">
                                                    <div className="w-full h-11 bg-muted/50 border border-border rounded-lg flex items-center px-4">
                                                        {filter.type === 'Status' ? (
                                                            <div className="flex items-center gap-2 bg-card px-3 py-1 border border-blue-200 rounded-md text-primary/90 text-sm font-bold shadow-sm">
                                                                {filter.value}
                                                            ...
                                                            <input 
                                                                ...
                                                                className="w-full bg-transparent border-none focus:ring-0 text-sm font-bold text-foreground/90 placeholder-gray-300"
                                                            />
                                                        )}
                            ...
                            <div className="flex items-center justify-between px-6 py-5 border-t border-gray-100 bg-background rounded-b-xl">
                                <div className="relative">
                                    <button 
                                        onClick={() => setIsAddFilterMenuOpen(!isAddFilterMenuOpen)}
                                        className="h-10 px-4 flex items-center gap-2 bg-card border border-border hover:border-primary/80 rounded-lg text-sm font-bold text-primary transition-all shadow-sm active:scale-95"
                                    >
                                        <Plus className="w-4 h-4" />
                                        Add Filter
                                        ...
                                        {isAddFilterMenuOpen && (
                                            <>
                                                ...
                                                <motion.div 
                                                    ...
                                                    className="absolute left-0 bottom-full mb-2 w-[240px] bg-card rounded-xl shadow-2xl border border-gray-100 z-[100] py-2 overflow-y-auto max-h-[350px] custom-scrollbar"
                                                >
                                                    {[
                                                        ...
                                                        <button 
                                                            ...
                                                            className="w-full text-left px-5 py-3 hover:bg-muted/50 text-[14px] font-bold text-foreground/90 transition-all flex items-center justify-between group"
                                                        >
                                                            {opt}
                                ...
                                <div className="flex items-center gap-3">
                                    <button onClick={() => setIsFiltersModalOpen(false)} className="h-10 px-6 bg-card border border-gray-300 rounded-lg text-sm font-black text-muted-foreground hover:text-foreground/90 transition-colors uppercase tracking-widest">Cancel</button>
                                    <button onClick={() => setIsFiltersModalOpen(false)} className="h-10 px-8 bg-primary hover:bg-primary text-white rounded-lg text-sm font-black transition-all shadow-xl shadow-primary/20 active:scale-95 uppercase tracking-[2px]">Apply</button>
                                </div>
                            </div>
                        </motion.div>
                    </div>
