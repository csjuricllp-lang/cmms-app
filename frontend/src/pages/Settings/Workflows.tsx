        ...
        <div className="p-8 space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1 font-medium">Create "If-Then" rules to automate your industrial operations</p>
                </div>
                <button 
                    onClick={() => setIsCreateModalOpen(true)}
                    className="flex items-center gap-2 bg-primary hover:bg-primary/90 text-white px-5 py-2.5 rounded-xl font-bold transition-all shadow-lg shadow-primary/20 active:scale-95"
                >
            ...
            <div className="grid grid-cols-3 gap-6">
                {[
                    ...
                    { label: 'Executed (24h)', value: '1,284', icon: Activity, color: 'text-primary', bg: 'bg-blue-50' },
                    { label: 'Time Saved', value: '42 hrs', icon: Clock, color: 'text-amber-600', bg: 'bg-amber-50' },
                ].map((stat, i) => (
                    <div key={i} className="bg-card p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
                        <div className={cn("p-3 rounded-xl", stat.bg)}>
                            <stat.icon className={cn("w-6 h-6", stat.color)} />
                        </div>
                        <div>
                            <p className="text-muted-foreground text-[13px] font-bold uppercase tracking-wider">{stat.label}</p>
            ...
            <div className="bg-card rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                <div className="p-6 border-b border-slate-50 flex items-center justify-between bg-transparent/50">
                    <div className="relative w-[300px]">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                        <input 
                            type="text" 
                            placeholder="Search workflows..."
                            className="w-full pl-10 pr-4 py-2 bg-card border border-border rounded-lg text-[14px] focus:border-primary/80 outline-none transition-all"
                            value={searchTerm}
                    ...
                    <div className="flex items-center gap-4 text-[13px] font-bold text-muted-foreground">
                        ...
                        <button className="hover:text-primary transition-colors">Sort by Name</button>
                    </div>
                </div>
                ...
                <div className="divide-y divide-slate-50">
                    {filteredRules.length > 0 ? (
                        filteredRules.map((rule: any) => (
                            <div key={rule.id} className="p-6 hover:bg-transparent transition-colors group">
                                <div className="flex items-center justify-between">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-3">
                                                ...
                                                {rule.isActive ? (
                                                    ...
                                                    <span className="text-[11px] font-black uppercase tracking-tighter bg-muted text-muted-foreground px-2 py-0.5 rounded-full">
                                                        Paused
                                                    </span>
                                                )}
                                            </div>
                                            <p className="text-muted-foreground text-[14px] mt-1">{rule.description || 'No description provided'}</p>
                                    ...
                                    <div className="flex items-center gap-6">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <button className="p-2 hover:bg-card hover:shadow-sm rounded-lg text-slate-400 hover:text-slate-600 transition-all">
    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            <motion.div 
                ...
                className="relative bg-card w-full max-w-[800px] rounded-3xl shadow-2xl overflow-hidden"
            >
                {/* Modal Header */}
                    ...
                    {step === 1 && (
                        <div className="space-y-8 animate-in slide-in-from-right-4 duration-500">
                            <div>
                                ...
                                <p className="text-muted-foreground font-medium">Give your workflow a clear name and description</p>
                            </div>
                            
                            <div className="grid grid-cols-1 gap-6">
                                <div className="space-y-2">
                                    <label className="text-[13px] font-black text-muted-foreground uppercase">Workflow Name</label>
                                    <input 
                                        type="text" 
                                        placeholder="e.g., High Priority Assignment"
                                        className="w-full px-5 py-4 border-2 border-slate-100 rounded-2xl font-bold text-slate-800 focus:border-primary/80 focus:ring-4 focus:ring-primary/10 outline-none transition-all"
                                        value={formData.name}
                                        onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                                    />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[13px] font-black text-muted-foreground uppercase">Description</label>
                                    <textarea 
                                        placeholder="Explain what this automation does..."
                                        rows={3}
                                        className="w-full px-5 py-4 border-2 border-slate-100 rounded-2xl font-bold text-slate-800 focus:border-primary/80 focus:ring-4 focus:ring-primary/10 outline-none transition-all resize-none"
                    ...
                    {step === 2 && (
                        <div className="space-y-8 animate-in slide-in-from-right-4 duration-500">
                            <div>
                                ...
                                <p className="text-muted-foreground font-medium">Select what event should start this workflow</p>
                            </div>
                                    ...
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            formData.entity === opt.entity && formData.trigger === opt.trigger
                                                ...
                                                : "border-slate-100 hover:border-border bg-card"
                                        )}
                                    >
                                        <div className={cn(
                                            "p-3 rounded-xl",
                                        ...
                                        <div>
                                            <p className={cn(
                                                ...
                                                formData.entity === opt.entity && formData.trigger === opt.trigger ? "text-indigo-900" : "text-foreground/90"
                                            )}>{opt.label}</p>
                                            <p className="text-[12px] text-muted-foreground font-bold uppercase">{opt.entity}</p>
                                        </div>
                                    </button>
                                ))}
                            </div>
                        ...
                        <div className="space-y-8 animate-in slide-in-from-right-4 duration-500">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-muted-foreground font-medium">Define what happens when the trigger fires</p>
                                </div>
                                <button 
                                    onClick={() => setFormData({ ...formData, actions: [...formData.actions, { type: 'NOTIFY_USER', value: '' }] })}
                                    className="flex items-center gap-2 bg-primary/10 text-primary px-4 py-2 rounded-xl font-black text-sm hover:bg-primary/15 transition-colors"
                                >
                                    <Plus className="w-4 h-4" />
                                    Add Action
                                </button>
                            ...
                            <div className="space-y-4">
                                {formData.actions.map((action, idx) => (
                                    <div key={idx} className="bg-transparent p-6 rounded-2xl border border-slate-100 flex items-center gap-4">
                                        <div className="flex-1 grid grid-cols-2 gap-4">
                                            <div className="space-y-1">
                                                <label className="text-[11px] font-black text-slate-400 uppercase">Action Type</label>
                                                <select 
                                                    className="w-full bg-card px-4 py-2 rounded-lg font-bold border border-border outline-none"
                                                    value={action.type}
                                                    onChange={(e) => {
                ...
                {/* Modal Footer */}
                <div className="p-8 bg-transparent border-t border-slate-100 flex items-center justify-between">
                    <button 
                        onClick={onClose}
                    ...
                    <div className="flex items-center gap-3">
                        {step > 1 && (
                            <button 
                                ...
                                className="px-8 py-3 rounded-2xl font-black text-slate-600 hover:bg-muted transition-all"
                            >
                                Back
                            </button>
                        )}
                        <button 
                            ...
                            className="bg-primary hover:bg-primary/90 text-white px-10 py-3 rounded-2xl font-black shadow-xl shadow-primary/20 transition-all active:scale-95 disabled:opacity-50 flex items-center gap-2"
                        >
