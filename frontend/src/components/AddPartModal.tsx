...
const AddPartModal: React.FC<AddPartModalProps> = ({ isOpen, onClose, workOrderId, onPartsAdded }) => {
    ...
    return (
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-[1100px] h-[85vh] bg-card rounded-[12px] shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                {/* Header Section */}
                <div className="px-8 pt-8 bg-card">
                    <div className="flex items-center justify-between mb-8">
                        <h2 className="text-[26px] font-[900] text-slate-800">Add Parts</h2>
                        <button onClick={onClose} className="p-2 hover:bg-muted/50 rounded-lg text-slate-400 transition-colors">
                            <X className="w-6 h-6" />
                        </button>
                    </div>
                ...
                {/* Toolbar Context */}
                <div className="px-8 py-5 border-b border-gray-100 bg-card">
                ...
                <div className="flex-1 overflow-auto custom-scrollbar bg-[#fafafa]">
                    {activeTab === 'Parts' ? (
                        <table className="w-full border-collapse bg-card">
                            <thead className="sticky top-0 bg-primary shadow-sm z-20">
                                <tr>
                                    <th className="p-5 text-left w-[60px]">
                                        <div className="w-5 h-5 border-2 border-border rounded cursor-pointer" />
                                    </th>
                                    <th className="p-5 text-left min-w-[200px] border-r border-gray-100">
                                        <div className="flex items-center gap-2 group cursor-pointer">
                                            <span className="text-[12px] font-black text-foreground/90 uppercase tracking-tight">Name</span>
                                            <ChevronDown className="w-4 h-4 text-slate-400 group-hover:text-slate-800 transition-colors" />
                                        </div>
                                    </th>
                                    <th className="p-5 text-left text-[12px] font-black text-foreground/90 uppercase tracking-tight">ID</th>
                                    <th className="p-5 text-left text-[12px] font-black text-foreground/90 uppercase tracking-tight">Location</th>
                                    <th className="p-5 text-left text-[12px] font-black text-foreground/90 uppercase tracking-tight">Area</th>
                                    <th className="p-5 text-left text-[12px] font-black text-foreground/90 uppercase tracking-tight">Cost</th>
                                    <th className="p-5 text-left text-[12px] font-black text-foreground/90 uppercase tracking-tight">Available Qty</th>
                                    <th className="p-5 text-left text-[12px] font-black text-foreground/90 uppercase tracking-tight">Maximum Qty</th>
                                    <th className="p-5 text-left text-[12px] font-black text-foreground/90 uppercase tracking-tight">Status</th>
                                    <th className="p-5 text-left text-[12px] font-black text-foreground/90 uppercase tracking-tight">Part Number</th>
                                    <th className="p-5 text-left text-[12px] font-black text-foreground/90 uppercase tracking-tight">Barcode</th>
                                </tr>
                            </thead>
                                ...
                                {isLoading ? (
                                    ...
                                    parts.map((part) => (
                                        <tr 
                                            key={part.id} 
                                            className={cn(
                                                "hover:bg-transparent transition-colors cursor-pointer group",
                                        ...
                                        >
                                            <td className="p-5">
                                                <div className={cn(
                                                    ...
                                                    selectedParts[part.id] ? "border-primary bg-primary" : "border-border"
                                                )}>
                                            ...
                                            <td className="p-5 border-r border-gray-100">
                                                <div className="flex flex-col">
                                                    <span className="text-[14px] font-bold text-foreground/90">{part.name}</span>
                                                    {selectedParts[part.id] !== undefined && (
                                                        <div className="mt-2 flex items-center gap-2 animate-in fade-in slide-in-from-left-2 duration-200">
                                                            <span className="text-[11px] font-black text-primary uppercase">Quantity:</span>
                                                            <input 
                                                                type="number"
                                                                min="1"
                                                                className="w-16 px-2 py-1 bg-blue-50 border border-blue-100 rounded text-[12px] font-black text-primary/90 outline-none focus:ring-2 focus:ring-blue-200"
                                            ...
                                            <td className="p-5">
                                                <span className="text-[14px] font-bold text-foreground/90">{part.cost?.toLocaleString() || '1.55'}</span>
                                            </td>
                                            <td className="p-5">
                                                <span className="text-[14px] font-bold text-foreground/90">{Number(part.quantity || 0).toFixed(2)}</span>
                                            </td>
                                            <td className="p-5">
                                                <span className="text-[14px] font-bold text-foreground/90">{part.maxQuantity || ''}</span>
                                            </td>
                                            <td className="p-5">
                                                <span className={cn(
                                                    "px-3 py-1 rounded-full text-[11px] font-black tracking-tight",
                                                    part.quantity <= 0 ? "bg-muted text-muted-foreground" :
                                            ...
                                            <td className="p-5 uppercase tracking-tighter">
                                                <span className="text-[13px] font-bold text-muted-foreground opacity-80">{part.partNumber || 'N/A'}</span>
                                            </td>
                                            <td className="p-5">
                                                <span className="text-[13px] font-bold text-muted-foreground font-mono opacity-80">{part.barcode || 'N/A'}</span>
                                            </td>
                                        </tr>
                                    ))
                                )}
                        ...
                        <div className="h-full flex flex-col items-center justify-center p-12 bg-[#fafafa]">
                            <div className="w-full max-w-md p-8 bg-blue-50/50 border border-blue-100 rounded-[20px] flex flex-col items-center text-center space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <div className="w-16 h-16 rounded-2xl bg-blue-100 flex items-center justify-center text-primary mb-2">
                                ...
                                <div className="pt-4 flex items-center gap-2 text-primary font-black text-[13px] uppercase tracking-widest cursor-pointer hover:underline">
                ...
                {/* Footer Section */}
                <div className="px-8 py-6 border-t border-gray-100 bg-card flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        <button 
                            onClick={onClose}
                            className="px-8 py-2.5 border border-border text-slate-600 rounded-lg text-[15px] font-black hover:bg-muted/50 transition-all active:scale-95"
                        >
                            Cancel
                        </button>
                        <button 
                            ...
                            className="px-8 py-2.5 bg-primary hover:bg-primary/90 disabled:opacity-50 text-white rounded-lg text-[15px] font-black shadow-lg shadow-primary/20 transition-all active:scale-95"
                        >
