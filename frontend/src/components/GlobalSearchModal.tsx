...
// ─── Type icons & colors ─────────────────────────────────────────────────────
const TYPE_CONFIG = {
    'work-order': { icon: ClipboardList, label: 'Work Order', color: 'text-primary',   bg: 'bg-blue-50',   border: 'border-blue-100' },
    'asset':      { icon: Box,           label: 'Asset',      color: 'text-primary', bg: 'bg-primary/10', border: 'border-violet-100' },
                ...
                <AnimatePresence>
                    {open && (
                        <>
                            ...
                            >
                                <div className="bg-card rounded-2xl border border-border shadow-[0_24px_60px_rgba(0,0,0,0.12)] overflow-hidden">
                                {/* Input row */}
                                <div className="flex items-center gap-3 px-4 py-3.5 border-b border-slate-100">
                                    ...
                                    {query && (
                                        <button
                                            ...
                                            className="w-6 h-6 flex items-center justify-center rounded-md bg-muted hover:bg-slate-200 transition-colors shrink-0"
                                        >
                                    ...
                                    <kbd
                                        onClick={() => setOpen(false)}
                                        className="px-2 py-1 text-[10px] font-bold text-slate-400 bg-muted rounded border border-border cursor-pointer hover:bg-slate-200 transition-colors font-mono"
                                    >
                                        ESC
                                    </kbd>
                                </div>
                                ...
                                <div className="max-h-[480px] overflow-y-auto p-1">
                                    ...
                                    {debouncedQuery.length < 2 && (
                                        <div className="px-4 py-8 text-center">
                                            <div className="w-12 h-12 rounded-2xl bg-muted/50 border border-border flex items-center justify-center mx-auto mb-3">
                                                <Search className="w-6 h-6 text-slate-300" />
                                            </div>
                                            <p className="text-[13px] font-semibold text-muted-foreground">Start typing to search</p>
                                    ...
                                    {isEmpty && (
                                        <div className="px-4 py-8 text-center">
                                            <p className="text-[13px] font-semibold text-muted-foreground">No results for</p>
                                            <p className="text-[15px] font-bold text-foreground/90 mt-1">"{debouncedQuery}"</p>
                                ...
                                {hasResults && (
                                    <div className="px-4 py-2.5 border-t border-slate-100 flex items-center gap-4 text-[10px] text-slate-400 font-medium bg-slate-50/70">
                                        <span className="flex items-center gap-1"><kbd className="px-1.5 py-0.5 bg-card rounded border border-border font-mono shadow-sm">↑↓</kbd> navigate</span>
                                        <span className="flex items-center gap-1"><kbd className="px-1.5 py-0.5 bg-card rounded border border-border font-mono shadow-sm">↵</kbd> open</span>
                                        <span className="flex items-center gap-1"><kbd className="px-1.5 py-0.5 bg-card rounded border border-border font-mono shadow-sm">ESC</kbd> close</span>
