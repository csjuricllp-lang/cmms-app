...
export const DraggableCard = ({ wo, onClick, onEdit, tagConfig }: DraggableCardProps) => {
    ...
    return (
        <div 
            ...
            onClick={onClick}
            className={cn(
                "bg-card border border-gray-100 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all group flex flex-col gap-4 cursor-grab active:cursor-grabbing relative",
                isDragging && "scale-105 shadow-xl border-primary/20"
            )}
        >
            <div className="space-y-4">
                <div className="flex-1 text-[13px] font-bold text-foreground/90 leading-snug group-hover:text-primary transition-colors pr-12">
                ...
                <div className="absolute top-4 right-4 flex items-center gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all">
                    <button 
                        ...
                        className="p-2 hover:bg-blue-50 rounded-lg text-primary transition-all"
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    {tagConfig?.filter(t => t.visible).map(tag => {
                        if (tag.id === 'status' && wo.status) {
                            return <span key={tag.id} className="px-2.5 py-1 bg-muted text-muted-foreground rounded-lg text-[9px] font-black uppercase tracking-tight border border-slate-200/50 truncate max-w-[140px]">{wo.status}</span>;
                        }
                        ...
                        if (tag.id === 'category' && wo.category) {
                            return <span key={tag.id} className="px-2.5 py-1 bg-primary/10 text-primary/90 rounded-lg text-[9px] font-black uppercase tracking-tight border border-purple-100 truncate max-w-[140px]">{wo.category}</span>;
                        }
                        return null;
                    })}
                </div>
            ...
            <div className="mt-auto flex items-center justify-between text-[11px] font-bold text-gray-400 uppercase tracking-widest pt-4 border-t border-gray-50/50">
                ...
                <div className="flex items-center gap-1.5 bg-muted/50 px-2 py-0.5 rounded-full border border-gray-100/50">
