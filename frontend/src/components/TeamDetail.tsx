...
export const TeamDetail: React.FC<TeamDetailProps> = ({ team, onClose, onEdit }) => {
    ...
    return (
        ...
        >
            ...
            <div className="bg-card border-b border-gray-100 px-6 py-4 flex items-center justify-between shadow-sm">
                <div className="flex items-center">
                    ...
                    <div className="flex items-center gap-6 px-6">
                        <button 
                            onClick={onClose}
                            className="p-2 hover:bg-muted/50 rounded-xl transition-all text-gray-400 hover:text-foreground"
                        >
                        ...
                        {team.isActive ? (
                            ...
                            <span className="flex items-center gap-1 text-[11px] font-bold text-slate-400 uppercase tracking-widest bg-muted/50 px-2 py-1 rounded-lg">
                ...
                <div className="flex items-center gap-3">
                    <button 
                        onClick={() => onEdit(team)}
                        className="flex items-center gap-2 px-5 py-2 border border-border rounded-xl text-[14px] font-black text-foreground/90 hover:bg-muted/50 transition-all shadow-sm active:scale-95"
                    >
                        Edit
                    </button>
                    <div className="relative">
                        <button 
                            ...
                            className={cn(
                                "p-2.5 hover:bg-muted/50 rounded-xl text-gray-400 transition-colors active:scale-95",
                                isMenuOpen && "bg-muted/50 text-foreground"
                            )}
                        >
                        ...
                        {isMenuOpen && (
                            <div className="absolute right-0 mt-2 w-48 bg-card border border-gray-100 rounded-2xl shadow-2xl z-[60] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                                <button 
                                    onClick={() => toggleStatusMutation.mutate()}
                                    disabled={toggleStatusMutation.isPending}
                                    className="w-full flex items-center gap-3 px-5 py-4 text-left text-foreground/90 hover:bg-muted/50 transition-all font-black text-[13px] border-b border-gray-100"
                                >
            ...
            {/* Tab Bar */}
            <div className="bg-card border-b border-gray-100 px-8">
                <div className="flex gap-8">
                    <button 
                        onClick={() => setActiveTab('details')}
                        className={cn(
                            "text-[13px] font-black py-4 uppercase tracking-widest transition-all",
                            activeTab === 'details' ? "text-foreground border-b-2 border-primary" : "text-gray-400 hover:text-gray-600"
                        )}
                    >
                        Details
                    </button>
                    <button 
                        ...
                        className={cn(
                            "text-[13px] font-black py-4 uppercase tracking-widest transition-all",
                            activeTab === 'resources' ? "text-foreground border-b-2 border-primary" : "text-gray-400 hover:text-gray-600"
                        )}
                    >
                        Resources
                    </button>
                ...
                <AnimatePresence mode="wait">
                    {activeTab === 'details' ? (
                        ...
                        >
                            ...
                            <div className="bg-card rounded-[32px] border border-gray-100 shadow-xl shadow-gray-200/50 overflow-hidden">
                                <div className="px-10 py-8 border-b border-gray-50">
                                    <h3 className="text-[17px] font-black text-foreground tracking-tight flex items-center gap-3">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="bg-card p-8 rounded-[32px] border border-gray-100 shadow-lg shadow-gray-200/40">
                                    <div className="flex items-center justify-between mb-6">
                                        <h4 className="text-[15px] font-black text-gray-800 uppercase tracking-widest">Personnel List</h4>
                                        <span className="bg-blue-50 text-primary px-3 py-1 rounded-full text-[11px] font-black">{team.users?.length || 0} Members</span>
                                    </div>
                                    <div className="space-y-4">
                                        {team.users?.map((user: any, i: number) => {
                                            const userData = user.userOrg?.user || user.user || {};
                                            return (
                                                <div key={i} className="flex items-center justify-between group cursor-pointer hover:bg-muted/50 p-2 rounded-2xl transition-all">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-gray-100 to-gray-200 flex items-center justify-center text-[12px] font-black text-muted-foreground">
                                                            {userData.name?.[0] || 'U'}
                                                        </div>
                                                        <div>
                                                            <p className="text-[14px] font-bold text-foreground">{userData.name || "Unknown Member"}</p>
                                ...
                                <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-8 rounded-[32px] text-white shadow-xl shadow-primary/20">
                            ...
                            {team.assets && team.assets.length > 0 ? (
                                <div className="grid grid-cols-1 gap-4">
                                    {team.assets.map((asset: any) => (
                                        <div key={asset.id} className="bg-card p-6 rounded-[28px] border border-gray-100 shadow-sm flex items-center justify-between group hover:shadow-md transition-all">
                                            <div className="flex items-center gap-4">
                                                <div className="w-12 h-12 bg-muted/50 rounded-2xl flex items-center justify-center text-primary">
                                                    <div className="w-6 h-6 border-2 border-current rounded" />
                                                </div>
                                                <div>
                                                    <p className="text-[15px] font-black text-foreground">{asset.name}</p>
                                ...
                                <div className="h-[50vh] flex flex-col items-center justify-center text-center space-y-6">
                                    <div className="w-24 h-24 bg-muted rounded-[32px] flex items-center justify-center text-gray-400">
                                        <Shield className="w-12 h-12 opacity-20" />
                                    </div>
                                    <div className="space-y-2">
                                        <h3 className="text-[20px] font-black text-foreground tracking-tight uppercase tracking-widest">No Resources Assigned</h3>
                                        <p className="text-[15px] font-medium text-muted-foreground max-w-sm mx-auto">
                                            Link this team to assets, specialized tools, or service vehicles to track their operational equipment.
                                        </p>
                                    </div>
                                    <button 
                                        onClick={() => setShowAssetPicker(true)}
                                        className="px-8 py-3.5 bg-primary text-white rounded-2xl text-[14px] font-black shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all active:scale-95"
                                    >
                                        Assign Equipment
                                    </button>
                                </div>
