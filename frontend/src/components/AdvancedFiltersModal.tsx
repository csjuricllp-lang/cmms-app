        ...
        <AnimatePresence>
            <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4 bg-slate-900/20 backdrop-blur-sm">
                <motion.div
                    ...
                    className="bg-card w-full max-w-[500px] rounded-[1.25rem] shadow-2xl flex flex-col relative"
                >
                    {/* Header */}
                    <div className="px-6 py-5 flex items-center justify-between border-b border-slate-100">
                        <h2 className="text-[18px] font-black text-foreground tracking-tight">Advanced Filters</h2>
                        <button 
                            onClick={onClose}
                    ...
                    <div className="min-h-[240px] max-h-[60vh] overflow-y-auto flex flex-col custom-scrollbar">
                        {visibleRows.length === 0 ? (
                            <div className="flex-1 flex flex-col items-center justify-center py-16 px-8 text-center">
                                ...
                                <h3 className="text-[15px] font-bold text-foreground mb-1">No filters added yet.</h3>
                            ...
                            <div className="p-6 space-y-6">
                                {visibleRows.map((category) => (
                                    <div key={category} className="space-y-2 group animate-in slide-in-from-top-2">
                                        ...
                                        <div className="h-12 border border-border rounded-xl flex items-center px-4 bg-transparent shadow-inner group-hover:border-slate-300 transition-colors">
                    ...
                    {/* Footer */}
                    <div className="px-6 py-5 border-t border-slate-100 flex items-center justify-between bg-transparent relative rounded-b-[1.25rem] overflow-visible">
                        <div className="relative">
                            <button 
                                onClick={() => setIsAddFilterOpen(!isAddFilterOpen)}
                                className="flex items-center gap-2.5 text-primary hover:text-primary/90 font-black text-[14px] transition-all px-3 py-2 rounded-xl hover:bg-blue-50/50"
                            >
                                ...
                                {isAddFilterOpen && (
                                    <>
                                        ...
                                        <motion.div 
                                            ...
                                            className="absolute bottom-full left-0 mb-2 w-56 bg-card border border-slate-100 rounded-2xl shadow-[0_20px_70px_-10px_rgba(0,0,0,0.15)] z-[120] py-2 ring-1 ring-black/5"
                                        >
                                            ...
                                            {ALL_CATEGORIES.map(category => (
                                                <button
                                                    ...
                                                    className="w-full text-left px-4 py-2.5 text-[14px] font-semibold text-slate-600 hover:bg-muted/50 hover:text-primary transition-all flex items-center justify-between group"
                                                >
                                                    {category}
                        ...
                        <div className="flex items-center gap-3">
                            <button 
                                onClick={onClose}
                                className="h-10 px-6 border border-border bg-card rounded-lg text-[14px] font-black text-slate-600 hover:bg-muted/50 transition-all shadow-sm active:scale-95 uppercase tracking-widest"
                            >
                                Cancel
                            </button>
                            <button 
                                onClick={handleApply}
                                className="h-10 px-8 bg-primary hover:bg-primary/90 text-white rounded-lg text-[14px] font-black transition-all shadow-lg shadow-primary/20 active:scale-95 uppercase tracking-[2px]"
                            >
                                Apply
                            </button>
                        </div>
