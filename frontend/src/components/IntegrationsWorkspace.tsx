...
export const IntegrationsWorkspace = () => {
    ...
    return (
        <div className="space-y-8 bg-card border border-[#E2E8F0] rounded-[24px] p-8 shadow-sm">
            <div>
                ...
                <p className="text-muted-foreground text-[13px] font-medium mt-1">Connect and synchronize your maintenance transactions with external ERP and accounting software.</p>
            </div>
                ...
                <div className="flex items-start justify-between">
                    <div className="space-y-1">
                        <div className="flex items-center gap-3">
                            ...
                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider ${
                                qbEnabled ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-muted text-slate-400'
                            }`}>
                                {qbEnabled ? 'Connected' : 'Disconnected'}
                            </span>
                        </div>
                        <p className="text-muted-foreground text-[13px] font-medium max-w-xl">
                    ...
                    <button 
                        ...
                        className={`w-12 h-6.5 rounded-full p-1 transition-all duration-300 outline-none flex items-center ${
                            qbEnabled ? 'bg-primary justify-end' : 'bg-slate-200 justify-start'
                        }`}
                    >
                        <div className="w-4.5 h-4.5 bg-card rounded-full shadow-md" />
                    </button>
                </div>
                ...
                {qbEnabled && (
                    <div className="pt-4 border-t border-slate-150 grid grid-cols-2 gap-6 animate-in slide-in-from-top-3 duration-300">
                        <div className="space-y-2">
                            <label className="text-[12px] font-black text-muted-foreground uppercase tracking-wider flex items-center gap-2">
                            ...
                            <input 
                                ...
                                className="w-full px-4 py-3 bg-card border border-border rounded-xl text-[13px] font-bold text-foreground/90 focus:border-primary/80 outline-none transition-colors"
                            />
                        </div>
                        ...
                        <div className="col-span-2 flex justify-end pt-2">
                            <button
                                onClick={handleSave}
                                disabled={isSaving}
                                className="px-6 py-2.5 bg-primary hover:bg-primary/90 disabled:opacity-40 text-white rounded-xl text-[13px] font-black shadow-lg shadow-primary/20 transition-all flex items-center gap-2"
                            >
            ...
            <div className="border border-dashed border-border rounded-2xl p-6 flex items-center gap-4 bg-slate-50/10">
