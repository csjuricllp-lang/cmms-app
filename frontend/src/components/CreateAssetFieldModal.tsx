    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6">
            ...
            <motion.div 
                ...
                className="relative w-full max-w-[480px] bg-card rounded-xl shadow-2xl overflow-hidden p-8 space-y-8"
            >
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-3">
                        <label className="text-[14px] font-bold text-foreground/90 ml-1">Field Name</label>
                        <input 
                            autoFocus
                            type="text"
                            value={fieldName}
                            onChange={(e) => setFieldName(e.target.value)}
                            className="w-full px-5 py-3.5 bg-card border-2 border-gray-100 rounded-xl text-[16px] font-medium focus:border-primary focus:ring-4 focus:ring-primary/10 outline-none transition-all"
                        />
                    </div>

                    {isNumber && (
                        <div className="space-y-3 pt-2">
                            <label className="text-[14px] font-bold text-foreground/90 ml-1">Decimal Precision</label>
                            <div className="relative group">
                                <select 
                                    value={precision}
                                    onChange={(e) => setPrecision(e.target.value)}
                                    className="w-full appearance-none px-5 py-3.5 bg-card border-2 border-gray-100 rounded-xl text-[16px] font-medium focus:border-primary focus:ring-4 focus:ring-primary/10 outline-none transition-all cursor-pointer"
                                >
                                ...
                                <ChevronDown className="absolute right-5 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 pointer-events-none group-focus-within:text-primary transition-colors" />
                            </div>
                        </div>
                    )}
                    ...
                    {isDropdown && (
                        <div className="space-y-6 pt-4">
                            ...
                            <div className="space-y-4">
                                <label className="text-[14px] font-bold text-foreground/90 ml-1">Options</label>
                                <div className="space-y-4">
                                    {options.map((option, index) => (
                                        <div key={index} className="flex items-center gap-3">
                                            ...
                                            <input 
                                                ...
                                                className="flex-1 px-4 py-2.5 bg-card border border-border rounded-lg text-[15px] font-medium focus:border-primary outline-none transition-all"
                                            />
                                            {options.length > 1 && (
                                ...
                                <button 
                                    onClick={addOption}
                                    className="flex items-center gap-2 px-4 py-2 text-[14px] font-bold text-muted-foreground hover:text-primary transition-colors"
                                >
                ...
                <div className="flex items-center justify-end gap-3 pt-4">
                    <button 
                        onClick={onClose}
                        className="px-8 py-3 bg-card border border-border text-slate-600 text-[14px] font-bold rounded-xl hover:bg-muted/50 transition-all"
                    >
                        Cancel
                    </button>
                    <button 
                        ...
                        className={cn(
                            ...
                            fieldName.trim() && (!isDropdown || options.every(o => o.trim()))
                                ...
                                : "bg-muted/50 text-slate-300 cursor-not-allowed"
                        )}
                    >
